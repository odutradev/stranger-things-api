import UtilCuriosities from "../util/curiosities.js";
import { sendError } from "../../src/app.js";

const { getFile, sendCurisioty } = new UtilCuriosities();
...
export default class CuriositiesController {
  ...
  async getCuriosityList(req, res) {
    ...
    await getFile((data, paste, index) => {
      if (data.length < start)
        return sendError(res, "curisioty_limit", data.length);
      data.forEach((x, i) => {
