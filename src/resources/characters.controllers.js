...
import StringSimilarity from "../util/checkSimilarity.js";
import UtilCharacters from "../util/characters.js";
import { sendError } from "../app.js";

const { didYouMean, checkSimilarity } = new StringSimilarity();
const { getFile, sendCharacter } = new UtilCharacters();
...
export default class CharactersController {
  ...
  getCharacterByName(req, res) {
    ...
    getFile(res, (data, paste, index) => {
      if (data.name.toLowerCase() == name) {
        ...
        var simularityName = didYouMean(
          checkSimilarity,
          name,
          pushNames.map((x) => x.name)
        );
  ...
  getCharacterList(req, res) {
    ...
    getFile(res, (data, paste, index) => {
      ...
      if (paste.length == index + 1)
        return res.status(200).json({ characters: characterList });
    });
  }
  ...
  async getCharacters(req, res) {
    ...
    if (characters.length < start)
      return sendError(res, "character_limit", characters.length);
    characters = [];
